...
def c_func(
    *, θ, norm_kepler_sq, a_0, C, v_r, B_φ, B_θ, B_r, ρ, η_A, η_O, η_H, b_φ,
    ...
    """
    return v_r ** 2 / 2 + 5/2 - norm_kepler_sq + a_0 * B_φ ** 2 / ρ + (
        a_0 * B_θ * C * (B_r / 4 + B_φ * tan(θ))
    ) / ρ + a_0 / (
        ρ * (η_O + η_A * (1 - b_φ ** 2))
    ) * (
        B_φ * (
...
def א_2_func(
    *, a_0, θ, v_r, ρ, B_φ, B_r, B_θ, deriv_B_r, deriv_B_θ, deriv_B_φ,
    ...
    return - v_r * A / 4 + (
        v_r ** 2 * C * A / 2 +
        4 * a_0 * B_φ / ρ * (B_φ * deriv_ρ / ρ - 2 * deriv_B_φ) +
        4 * a_0 / ρ * (
            deriv_ρ * B_θ * C * (B_r / 4 + B_φ * tan(θ)) / ρ -
            deriv_B_θ * C * (
                B_r / 4 + B_φ * tan(θ)
            ) - B_θ * (
                A * (
                    ...
                    deriv_B_r / 4 + deriv_B_φ * tan(θ) + B_φ * sec(θ) ** 2
                )
            ) + (
                deriv_ρ / ρ * (
                    B_φ * (
                    ...
                    deriv_η_A * b_φ * (b_θ / 4 + b_r * tan(θ)) +
                    η_A * deriv_b_φ * (b_θ / 4 + b_r * tan(θ)) +
                    η_A * b_φ * (
                        deriv_b_θ / 4 + deriv_b_r * tan(θ) + b_r * sec(θ)**2
                    ) - deriv_η_H * (b_r / 4 + b_θ * tan(θ)) -
                    η_H * (
...
def Z_func(
    *, a_0, B_r, B_φ, B_θ, η_O, η_H, η_A, θ, v_r, v_φ, ρ,
    ...
    return (
        ...
        C / 3 - 2 / (3 * sqrt(b ** 2 - 4 * c)) * (
            v_r * (C ** 2 / 4 - 4) +
            2 * a_0 * B_θ / (ρ * (η_O + η_A * (1 - b_φ ** 2)))
        )
    ) / (
        2 * a_0 / (v_φ ** 2 * ρ) * (
            B_θ * deriv_B_φ - B_r * B_φ / 4 - B_θ * B_φ * tan(θ)
