...
from ..file_format import ConfigInput
from ..file_format import InitialConditions
from ..float_handling import FLOAT_TYPE
from ..float_handling import float_type
from ..utils import CaseDependentConfigParser
from ..utils import ODEIndex
from .utils import add_overrides
...
def v_φ_boundary_func(*, v_r, η_H, η_P, norm_kepler_sq, a_0, γ):
    ...
    """
    try:
        return v_r * η_H / (4 * η_P) + sqrt(norm_kepler_sq - 5 / 2 + 2 * γ +
                                            v_r * (a_0 / η_P + v_r / 2 *
                                                   (η_H**2 /
                                                    (8 * η_P**2) - 1)))
    except ValueError:
        ...
        log.error(w)
        raise SolverError("Cannot take sqrt")
...
def define_conditions(inp):
    ...
    return InitialConditions(norm_kepler_sq=norm_kepler_sq,
                             a_0=a_0,
                             init_con=init_con,
                             angles=angles,
                             γ=γ)
...
def get_input_from_conffile(*, config_file, overrides=None):
    ...
    return add_overrides(
        ...
        config_input=ConfigInput(
            ...
            taylor_stop_angle=config.get("config",
                                         "taylor_stop_angle",
                                         fallback="0.001"),
            ...
            relative_tolerance=config.get("config",
                                          "relative_tolerance",
                                          fallback="1e-6"),
            absolute_tolerance=config.get("config",
                                          "absolute_tolerance",
                                          fallback="1e-10"),
            jump_before_sonic=config.get("config",
                                         "jump_before_sonic",
                                         fallback="None"),
            v_θ_sonic_crit=config.get("config",
                                      "v_θ_sonic_crit",
                                      fallback="None"),
            after_sonic=config.get("config", "after_sonic", fallback="None"),
            interp_range=config.get("config", "interp_range", fallback="10"),
            sonic_interp_size=config.get("config",
                                         "sonic_interp_size",
                                         fallback="None"),
            interp_slice=config.get("config",
                                    "interp_slice",
                                    fallback="-1000,-100"),
            ...
            mcmc_vars=config.get("config", "mcmc_vars",
                                 fallback="v_r,v_a,v_k"),
            target_velocity=config.get("config",
                                       "target_velocity",
                                       fallback="0.9"),
            split_method=config.get("config",
                                    "split_method",
                                    fallback="v_θ_deriv"),
            use_taylor_jump=config.get("config",
                                       "use_taylor_jump",
                                       fallback="True"),
